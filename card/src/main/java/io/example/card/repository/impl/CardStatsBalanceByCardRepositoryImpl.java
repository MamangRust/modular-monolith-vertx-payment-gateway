...
public class CardStatsBalanceByCardRepositoryImpl implements CardStatsBalanceByCardRepository {
  ...
  public CardStatsBalanceByCardRepositoryImpl(Pool pool) {
    this.pool = pool;
  }
  ...
  @Override
  public Future<List<CardStats.MonthBalance>> getMonthlyBalancesByCardNumber(int year, String cardNum) {
    return pool.preparedQuery("""
        ...
        """).execute(Tuple.of(getYearStart(year), cardNum)).map(this::mapMonthBalanceList);
  }

  @Override
  public Future<List<CardStats.YearlyBalance>> getYearlyBalancesByCardNumber(int endYear, String cardNum) {
    return pool.preparedQuery("""
        ...
        """).execute(Tuple.of(endYear, cardNum)).map(this::mapYearBalanceList);
  }
