...
class CardDashboardBalanceRepositoryImplTest {
  ...
  private CardDashboardBalanceRepositoryImpl repo;
  @BeforeEach void setUp() { repo = new CardDashboardBalanceRepositoryImpl(pool); }

  @Test void getTotalBalances(VertxTestContext ctx) {
    ...
    repo.getTotalBalances().onComplete(ctx.succeeding(v -> ctx.verify(() -> { assertThat(v).isEqualTo(10000L); ctx.completeNow(); })));
  }

  @Test void getTotalBalanceByCardNumber(VertxTestContext ctx) {
    when(pool.preparedQuery(anyString())).thenReturn(pq);
    when(pq.execute(any(Tuple.class))).thenReturn(Future.succeededFuture(rs));
    when(rs.iterator()).thenReturn(it); when(it.hasNext()).thenReturn(true); when(it.next()).thenReturn(row);
    when(row.getLong(0)).thenReturn(5000L);
    repo.getTotalBalanceByCardNumber("4111").onComplete(ctx.succeeding(v -> ctx.verify(() -> { assertThat(v).isEqualTo(5000L); ctx.completeNow(); })));
  }
}
