...
class MerchantStatsAmountByMerchantRepositoryImplTest {
  ...
  private MerchantStatsAmountByMerchantRepositoryImpl repo;
  private MonthYearAmountMerchant req = MonthYearAmountMerchant.builder().merchantId(5).year(2026).build();
  @BeforeEach void setUp() { repo = new MerchantStatsAmountByMerchantRepositoryImpl(pool); }
  void m() { when(pool.preparedQuery(anyString())).thenReturn(pq); }

  @Test void getMonthlyAmountByMerchants(VertxTestContext ctx) {
    m();
    ...
    repo.getMonthlyAmountByMerchants(req).onComplete(ctx.succeeding(l -> ctx.verify(() -> { assertThat(l).hasSize(1); ctx.completeNow(); })));
  }

  @Test void getYearlyAmountByMerchants(VertxTestContext ctx) {
    m();
    when(row.getValue("year")).thenReturn("2025"); when(row.getLong("amount")).thenReturn(6000L);
    when(rs.iterator()).thenReturn(it); when(it.hasNext()).thenReturn(true, false); when(it.next()).thenReturn(row);
    when(pq.execute(any(Tuple.class))).thenReturn(Future.succeededFuture(rs));
    repo.getYearlyAmountByMerchants(req).onComplete(ctx.succeeding(l -> ctx.verify(() -> { assertThat(l).hasSize(1); ctx.completeNow(); })));
  }
}
