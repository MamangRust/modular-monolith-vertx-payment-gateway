...
class TopupStatsByCardMethodRepositoryImplTest {
  ...
  private TopupStatsByCardMethodRepositoryImpl repo;
  ...
  @BeforeEach
  void setUp() {
    repo = new TopupStatsByCardMethodRepositoryImpl(pool);
  }
  ...
  @Test
  @DisplayName("getMonthlyTopupMethodsByCard returns list of monthly method stats by card")
  void monthly(VertxTestContext ctx) {
    ...
    repo.getMonthlyTopupMethodsByCard(req)
  ...
  @Test
  @DisplayName("getYearlyTopupMethodsByCard returns list of yearly method stats by card")
  void yearly(VertxTestContext ctx) {
    ...
    repo.getYearlyTopupMethodsByCard(req)
        .onComplete(ctx.succeeding(l -> ctx.verify(() -> {
          assertThat(l).hasSize(1);
          ctx.completeNow();
        })));
