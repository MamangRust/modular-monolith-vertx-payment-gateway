...
public class TransactionStatsMethodRepositoryImpl implements TransactionStatsMethodRepository {
  ...
  public TransactionStatsMethodRepositoryImpl(Pool pool) {
    this.pool = pool;
  }
  ...
  @Override
  public Future<List<TransactionStats.MonthMethod>> getMonthlyMethodsByCard(FindByYearCardNumberTransactionRequest request) {
    String sql = """
        WITH months AS (
            SELECT generate_series(
  ...
  @Override
  public Future<List<TransactionStats.YearMethod>> getYearlyMethodsByCard(FindByYearCardNumberTransactionRequest request) {
    int endYear = (int) request.getYear();
    String sql = """
        SELECT
            EXTRACT(YEAR FROM t.created_at)::integer AS year,
